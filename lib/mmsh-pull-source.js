...
exports.MMSHPullSource = function(http_params) {
    ...
    /**
      ...
      */
    this.initiate = function(connect_host, connect_port) {
	var http_client = http.createClient(connect_port, connect_host);

	http_client.on("error", onError);
	...
	req.once('response', function(res) {
		...
		log.error(c, "MMSH stream terminated!");
	    });
	...
	if(hsp_util.isLocalIP(address) && !(hsp_util.config.use_proxy_for_local_addresses)) {
	    ...
	    this.initiate(http_params.host, http_params.port);
	} else {
	    log.debug(c, "Pull Source address is not local (or proxy forced), using proxy.");
	    this.initiate(hsp_util.config.http_proxy_host, hsp_util.config.http_proxy_port);
	}
    };
	...
	if(net.isIPv4(http_params.host)) {
		...
		if(err === null) {
		    ...
		    onError("Couldn't look up pull source hostname: " + err);
		}
	    }.bind(this));
	}
    } else {
